...
import type { ExportResult, JSONValue, LoggingControlDescriptor, ScoreQsosResult } from '@ham2k/extension-sdk'
import { CQWWScorer } from './scorer.ts'
import { CANADIAN_QTHS, US_QTHS, normalizeQth, qthForCall, qthMultiplier } from './rtty.ts'
...
function qso(call: string, zone = '5', qth = 'MA', band = '20m', mode = 'RTTY'): Qson {
  return { uuid: `${call}-${band}-${mode}`, startAtMillis: Date.UTC(2026, 8, 26, 12),
    freq: 14085, band, mode, their: { call }, refs: [{ type: 'cqww', theirZone: zone, theirQth: qth }] }
}
function run(qsos: Qson[], op = operation, contestRef = ref) {
  ...
  const scores = qsos.map(qso => {
    const result = CQWWScorer.scoreQso({ scoresheet: sheet, qso, operation: op, ref: contestRef, isNewDay: false }, ctx)
...
async function exported(qsos: Qson[], op = operation): Promise<ExportResult> {
  return await cqww.runHook('export', 'generateExport', { operation: op, qsos, exportType: 'cqww-cabrillo' }) as ExportResult
}

test('RTTY scores 1/2/3 points and all three multiplier axes', () => {
  const { sheet, scores } = run([qso('W1AW', '5', 'CT'), qso('VE3XYZ', '4', 'ON'), qso('DL1ABC', '14', 'DX')])
  assert.deepEqual(scores.map(s => s.value), [1, 2, 3])
  assert.equal(Object.keys(sheet.mults).length, 8)
  const summary = CQWWScorer.summarizeScore({ scoresheet: sheet, operation, ref, scope: 'operation' }, ctx).cqww
...
test('RTTY duplicates persist across UTC days, but a new band counts', async () => {
  ...
  const anotherBand: Qson = { ...qso('W1AW', '05', 'CT', '40m'), startAtMillis: Date.UTC(2026, 8, 27, 13) }
  const result = await cqww.runHook('scoring', 'scoreQsos', { operation, ref, qsos: [first, nextDay, anotherBand] }) as ScoreQsosResult
...
test('RTTY sidebands share the dupe key; generic DATA and FT8 are not RTTY', () => {
  const { scores } = run(['RTTY-LSB', 'RTTY-USB', 'RTTY-R', 'DATA', 'DATA-USB', 'FT8'].map(mode => qso('W1AW', '5', 'CT', '20m', mode)))
...
test('160m and WARC do not count in RTTY; single-band scores leave other bands unclaimed', async () => {
  ...
  assert.deepEqual(run(qsos, operation, single).scores.map(s => s.value), [1, 0])
...
test('CQ country multipliers use WAE exceptions including Sicily and African Italy', () => {
  const { sheet, scores } = run([qso('I2ABC', '15', 'DX'), qso('IT9ABC', '15', 'DX'), qso('IG9ABC', '33', 'DX'), qso('IH9ABC', '33', 'DX')])
  assert.deepEqual(scores.map(s => s.value), [3, 3, 3, 3])
...
test('missing, cleared, invalid, or mismatched exchanges do not consume a valid contact', () => {
  const bad = [qso('W1AW', '', 'CT'), qso('W1AW', '41', 'CT'), qso('W1AW', '5', ''), qso('W1AW', '5', 'ON'), qso('W1AW', '5', 'DX')]
  ...
  const missing = qso('DL1ABC', '14', 'DX'); delete missing.refs
...
test('QTH suggestions are tentative lookup data, never guesses from call digits', async () => {
  const get = async (candidate: Qson) => await cqww.runHook('activity', 'loggingControls', { operation, qso: candidate }) as LoggingControlDescriptor[]
  ...
  const cw = await cqww.runHook('activity', 'loggingControls', { operation: { ...operation, refs: [{ ...ref, mode: 'CW' }] }, qso: qso('W1AW') }) as LoggingControlDescriptor[]
  assert.equal(cw.length, 1)
})

test('save projection and ADIF preserve corrections, aliases and deliberate clearing', async () => {
  const candidate = { ...qso('VE3XYZ', '3', 'nt'), their: { call: 'VE3XYZ', guess: { state: 'ON', cqZone: 4 } } }
  const patch = await cqww.runHook('activity', 'processQsoBeforeSave', { operation, qso: candidate }) as Qson
  ...
  const blank = await cqww.runHook('activity', 'processQsoBeforeSave', { operation, qso: cleared }) as Qson
  assert.deepEqual(blank.their, { exchange: '' })
  assert.deepEqual(blank.refs, [{ type: 'cqww', theirZone: '', theirQth: '' }])
  const fields = await cqww.runHook('adifFields', 'fieldsForOneQSO', { operation, qso: candidate }) as { name: string; value: string }[]
...
test('Cabrillo includes RY, padded zones, both QTH columns and category headers', async () => {
  const file = await exported([qso('VE3XYZ', '4', 'ON', '20m', 'RTTY-USB'), qso('DL1ABC', '14', 'DX'), qso('W1AW', '5', 'CT', '20m', 'FT8')])
  ...
  assert.deepEqual(lines[0].trim().split(/\s+/), ['QSO:', '14085', 'RY', '2026-09-26', '1200', 'N1RWJ', '599', '05', 'MA', 'VE3XYZ', '599', '04', 'ON'])
...
test('Cabrillo honors received RSTs and refuses an incomplete sent exchange', async () => {
  const candidate = { ...qso('W1AW', '05', 'CT'), our: { sent: '579' }, their: { call: 'W1AW', sent: '589' } }
  assert.ok((await exported([candidate])).content.includes('N1RWJ 579 05 MA W1AW 589 05 CT'))
  for (const setup of [{ ...ref, qth: '' }, { ...ref, zone: '' }, { ...ref, qth: 'ON' }]) {
    await assert.rejects(exported([candidate], { ...operation, refs: [setup] }), /Configure a valid CQ zone/)
  }
})
...
test('RTTY references link to their own rules and show the sent QTH', async () => {
  ...
  const title = await cqww.runHook('ref:cqww', 'suggestOperationTitle', { operation, ref }) as { subtitle: string }
...
test('CW/SSB Cabrillo exchange columns and rules links stay compatible', async () => {
  ...
  const link = await cqww.runHook('ref:cqww', 'linkForRef', { ref: { type: 'cqww', mode: 'CW' } }) as { url: string }
