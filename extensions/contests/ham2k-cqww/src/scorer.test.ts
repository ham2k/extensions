...
import { CQWWScorer, normalizeZone } from "./scorer.ts"
...
function qso(call: string, band = '20m', mode = 'SSB', theirZone?: string): Record<string, JSONValue> {
  return {
    their: { call },
    band,
    mode,
...
function run(qsos: Record<string, JSONValue>[], operation = usOperation, ref: Record<string, JSONValue> = ssbRef) {
  let sheet = CQWWScorer.startScoresheet({ operation, ref }, ctx)
  const scores = qsos.map((q) => {
    const r = CQWWScorer.scoreQso({ scoresheet: sheet, qso: q, operation, ref, isNewDay: false }, ctx)
    sheet = r.scoresheet
    return r.score
  })
  return { sheet, scores }
...
test('a zone the operator deliberately emptied earns no multiplier', () => {
  ...
  const { scores, sheet } = run([
    { their: { call: 'DL1ABC' }, band: '20m', mode: 'SSB', refs: [{ type: 'cqww', theirZone: '' }] },
  ])
...
test('a QSO in the wrong mode or on a WARC band does not count', () => {
  ...
  const { scores: wrongMode } = run([qso('DL1ABC', '20m', 'CW')], usOperation, { type: 'cqww', mode: 'SSB' })
...
test('the score is points times multipliers, not points alone', () => {
  ...
  const summary = CQWWScorer.summarizeScore({ scoresheet: sheet, operation: usOperation, scope: 'operation' }, ctx)
