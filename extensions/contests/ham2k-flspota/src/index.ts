...
//
// Florida State Parks on the Air — the Tamiami Amateur Radio Club's April event
// (https://fspota.org).
//
...
// extension carry `{type: 'stateparks', ref: 'FLSP'}` instead, and are answered
...
// in scorer.ts.
//
// What the sponsor takes is one ADIF per park activated, named for the park
// (§7.5.1, §7.5.3), so that file is ours to write rather than POTA's. Entry and
// power classes and two of the three bonuses are claimed on the sponsor's upload
// form and change nothing this produces, so setup asks for none of them.
...
import { END_MILLIS, LEGACY_CLAIM, LEGACY_KEY, LEGACY_TYPE, NAME, START_MILLIS, TYPE, URL } from "./event.ts"
import { tFor } from "./i18n.ts"
import { POTA_ACTIVATION, eventRefIn, ourParks } from "./parks.ts"
import { FlspotaScorer } from "./scorer.ts"

import manifest from "../manifest.json" with { type: "json" }

const DAY_MILLIS = 24 * 60 * 60 * 1000

/// `Florida State Parks on the Air 2026`. The year is the one the dates name,
...
/// `2026-04-17 12:00Z`. Written out rather than localized: these are the
...
const ActivityHook = {
  ...
  async suggest({ searchTerm, scoped }: SuggestArgs, _ctx: HookContext): Promise<ActivitySuggestion[]> {
    ...
    const namesUs = !term
      ...
      // Equality, because a state code is two characters: `FL` inside a longer
      // word would answer for Florida in half the searches an operator types.
      || term === 'FL'
...
const RefHandler = {
  ...
  /// "KI2D for FSPOTA". The park is POTA's to put in the title.
...
/// `W1RCP@US-2195-20260418.adi` — §7.5.1's own example: "a filename formatted as
/// for POTA: CALL@USPARK_ID-DATE". The slash of a portable callsign becomes a
/// dash.
...
const ExportHook = {
  ...
  /// One file per listed park — §7.5.3: "One ADIF format log file must be
  /// submitted for each park activated. Combined log files for multiple parks
  ...
  async generateExport(args: ExportRequest, _ctx: HookContext): Promise<ExportResult> {
    ...
    const content = await adifForExport({
      ...
      // contributes nothing. Its one-record-per-hunted-park is what §7.4's "POTA
      // log format" asks for, and what the scorer counts (scorer.ts).
...
defineExtension({
  ...
  onActivation({ registerHook }) {
    ...
    registerHook('scoring', {
      hook: contestScorer(FlspotaScorer, { scope: { refTypes: [TYPE, LEGACY_TYPE] } }),
      key: manifest.key,
    })
  },
})
