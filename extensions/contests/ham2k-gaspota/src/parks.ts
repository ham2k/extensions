...
//
// The event has no exchange of its own (§4.5: a signal report, "with or without
// the park identifier") and its logs name parks by POTA number (§7.2.1), so the
// parks are the POTA refs the operator is already logging: `potaActivation` on
// the operation, `pota` on the QSO.
...
/// The listed Georgia parks we are operating from. [operation] is the
